...
class Trainer:
    ...
    def validate(self, epoch):
        ...
        # Convert final_preds, final_targets to single tensor
        for task in ['condition', 'region', 'depth']:
...
def test_model(model, test_loader, device, 
    ...
    # For multi-class tasks (region), we can choose 'macro' or 'weighted' average, etc.
    # For binary tasks (condition, depth), 'binary' or 'macro' is typically fine.
    ...
    # Return a dictionary if you'd like to track these programmatically
    return {
...
def objective(trial):
    ...
    # Example: You could also tune dropout probabilities or any other hyperparams
    # E.g., dropout in the final heads. We can’t just pass them blindly but demonstrate:
    dropout_head  = trial.suggest_float('dropout_head', 0.3, 0.6)

    # We'll fix other parameters for demonstration
    max_epochs    = 15    # keep smaller for faster tuning
    patience      = 5     # early stopping patience

    # -----------------------------
    # 2) Set up data
    # -----------------------------
    # IMPORTANT: For large datasets, you typically shouldn't reload the entire dataset 
    # for each trial. Instead, you might do it once outside of objective(). 
    # For this example, we’ll do a minimal approach.

    base_dir = Path("C:/Kelvin_ASD_v2")  # Adjust path to your data
    metadata_df = pd.read_csv('metadata/dataset_metadata.csv')

    # You could do KFold inside the objective if you want to get a cross-validation metric 
    # for each trial. Or just do a single train/val split. 
    # For brevity, let's do a single split here:
    ...
    # We'll just do a single fold training. 
    ...
    # Optionally, if you want to change dropout in the final heads:
    # E.g., directly modify the .Dropout(0.5) with the suggested dropout_head
    # NOTE: For a more robust approach, you'd define a custom constructor that accepts
    #       dropout parameters. We'll do a quick hacky approach:
    for block in [model.condition_head, model.region_head, model.depth_head]:
        # block is nn.Sequential
        # let's assume each block has [nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(...), nn.ReLU(), nn.Dropout(...), nn.Linear(...)]
        # We can replace the dropout layer:
    ...
    # We can give a unique experiment name for each trial
    ...
    # We'll use the final (or best) validation loss from history as the objective
    best_val_loss = min(history['val_loss'])
    
    # IMPORTANT: Optuna tries to *minimize* the returned metric by default. 
    # If you prefer to maximize accuracy, you'd return -val_accuracy or set `direction="maximize"` in study.
...
def main():
    ...
    # -----------------------------
    # 5) (Optional) Re-train or load the best model with best hyperparams
    # -----------------------------
    # If you want, you can now create a fresh model with the best trial's hyperparams
    best_hparams = best_trial.params

    print("\nRe-building the best model to test on hold-out set...")

    # (Re)load your dataset metadata as needed
    # For demonstration, let's do it again:
    base_dir = Path("C:/Kelvin_ASD_v2")
    metadata_df = pd.read_csv('metadata/dataset_metadata.csv')
    ...
    model = MultiTaskCNN().to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    
    # Example: apply best dropout
    ...
    # Create criterion & optimizer with best hyperparams
    criterion = MultiTaskLoss(weights={'condition': 1.0, 'region': 1.0, 'depth': 1.0})
    ...
    # Optionally, re-train your best model using the entire (train + val) dataset or a bigger fraction.
    # For demonstration, let's just load from the best_model.pth if it was saved, or you can do:

    # (Re)train the best model on full data if you wish:
    # ...
    # trainer = Trainer(...)
    # trainer.train()

    # Then test
    # If you have a saved best_model, load it:
    # checkpoint = torch.load('best_model.pth')
    # model.load_state_dict(checkpoint['model_state_dict'])

    # If not, let's assume we've just trained it. 
